...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Stack<Player> winners = findWinners(players, scoreService);
        if(winners.size() > 1){
    ...
    public static Stack<Player> findWinners(Stack<Player> players, ScoreService scoreService) {
        HandComparison comparedHands = new HandComparison();
        Stack<Player> winners = new Stack<Player>();
        for(Player player: players){
            if(comparedHands.getWinner() == null){
                comparedHands.setWinner(player.getBestHand());
                winners.push(player);
                continue;
            }
